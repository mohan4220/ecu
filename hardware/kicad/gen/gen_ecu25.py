...
# Name the hold-up node. It used to be the autogenerated Net-(D3-K), which
# meant the PCB netclass rule and the DRU keyed off a reference designator:
# renumbering D3 would have silently dropped the rail to the Default class.
s.label((vin[0] - 12, vin[1]), "+12V_HLD")
...
# --- Type-3 ripple injection (LM5164 datasheet SNVSAU4D, Table 6-1 / 7.2.2.6)
# A COT regulator's feedback comparator needs >=20 mV of ripple at FB that is
# IN PHASE with the inductor current. An all-ceramic output gives ~2.7 mV and
# that component is capacitive, i.e. out of phase, so the part burst-switches:
# several on-times in quick succession followed by a long off-time. RA/CA
# build a triangular ramp from the SW node and CB couples it into FB, so the
# ripple exists only at the feedback pin and the output stays quiet.
#   CA >= 10 / (Fsw * (Rfb1||Rfb2)) = 10 / (303k * 9.20k) = 3.6 nF -> 4.7 nF
#   RA <= tON * (Vin - Vout) / (20mV * CA)
#      = 1.31us * 7.6 / (20m * 4.7n) = 106k -> 100k
#      giving 21 mV at 12.6 V and 13 mV at 8 V (datasheet floor is 12 mV)
#   CB >= t_settle / (3 * Rfb1) = 75us / 115k = 0.65 nF -> 1 nF C0G
_l_left = l2.pin(2) if l2.pin(2)[0] < l2.pin(1)[0] else l2.pin(1)
assert _l_left[0] - c4.pin(2)[0] > 5.08, "SW run too short to label"
s.label((c4.pin(2)[0] + 5.08, sw[1]), "BUCK_SW")
c10 = C(s, "C10", "4.7nF C0G", 200, 140)
rail(s, "+5V", c10.pin(1))
r7 = R(s, "R7", "100k 1%", 200, 152)
s.wire(c10.pin(2), r7.pin(1))
s.wire(r7.pin(2), (r7.pin(2)[0], r7.pin(2)[1] + 6))
s.label((r7.pin(2)[0], r7.pin(2)[1] + 6), "BUCK_SW")
c11 = C(s, "C11", "1nF C0G", 216, 150)
s.wire(c10.pin(2), (216, c10.pin(2)[1]))
s.junction(c10.pin(2))
s.wire((216, c10.pin(2)[1]), c11.pin(1))
s.wire(c11.pin(2), (c11.pin(2)[0], c11.pin(2)[1] + 6))
s.label((c11.pin(2)[0], c11.pin(2)[1] + 6), "BUCK_FB")
...
s.text((20, 18), "8x 12V digital inputs (divider + clamp, fw debounce);"
...
for i in range(8):
    ...
    r13 = R(s, f"R{231+i}", "1k 0.5W", x0 + 2, yc - 18)
...
s.text((20, 18), "6x G5LE-1-DC12 relays, 2N7002K low-side drivers, SS34 flyback."
...
FP_BY_REF = {
    ...
    "C9":  "Capacitor_THT:CP_Radial_D12.5mm_P5.00mm",      # 2200uF 35V crank
                                                           # hold-up bank
