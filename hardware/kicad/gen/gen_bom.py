...
RATING = {
    ...
    "10k 0.5W": "0.5 W / 1210 — DIN divider top leg",
    "0.05R 3W": "3 W 2512 current-sense (Kelvin pads), 1 %",
    "220R 5W": "5 W axial cement — D+ excitation, dissipates 2.6 W in fault",
    "100nF 100V": "X7R 100 V — MPU AC coupling",
    "2.2uF 100V": "X7R 100 V 1210 — buck input",
    ...
    "100k": "R4 in this line is the buck RON resistor: 100 k sets fsw ≈ 300 kHz",
...
PARTS = {
    ...
    "F2":  ("RXEF110", "1.1 A hold / 2.2 A trip PPTC, 72 V radial — it must stand off "
                       "the ~53 V load-dump clamp while tripped, and no chip PPTC does "
                       "that at 1.1 A hold"),
    "C76": ("0.22 F 5.5 V EDLC coin", "RTC backup, radial D10/P5.0"),
    "C2":  ("100 nF 100 V X7R", "+24 V bypass — 100 V for the same clamp reason as C1"),
    "FB2": ("BLM18PG600SN1", "ferrite bead 0603 — VDDA filter"),
    "C1":  ("100 uF 63 V radial", "input bulk — 63 V, not 50 V: the rail clamps at ~53 V"),
    "D1":  ("SMCJ33CA", "bidirectional input TVS"),
    "D2":  ("BZT52C15", "gate zener"),
    "D70": ("BAT54GWX", "supercap isolation Schottky"),
    "D80": ("SMBJ33CA", "bidirectional, +24V_SW local clamp"),
...
FAMILY = {
    ...
    "G5LE-1 24V": ("G5LE-1-DC24", "Omron 10 A SPDT, 24 V coil"),
    "600R@100MHz 3A": ("BLM21PG600SN1", "ferrite bead 0805, 3 A — series in the +24 V feed"),
}
